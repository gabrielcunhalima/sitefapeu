...
@php 
  ...
  $auxdata = explode(" ",$evento_config->dataInicioInscricao);
  $datainicio = $auxdata[0]; 
  $horainicio = $auxdata[1];
  $auxdata = explode(" ",$evento_config->VencimentoPagamento);
  $datafim = $auxdata[0]; 
  $horafim = $auxdata[1]; 
@endphp
<div>
  <h5 class='center'>Configurações [ID: {{$evento->id}}]</h5>
  <form class='col s12' action='{{route('eventos.editconfig')}}' method="POST" enctype="multipart/form-data">
        ...
        <div  class="input-field col s5">
          <label class="corCheckbox">
            <span>Quantidade total de Vagas</span>
                <input id="TotalVagasGeral" type="number" class="validate" name='TotalVagasGeral' {{$evento->TotalVagasGeral}} >
            </label>
        </div>
        <div class="input-field col s12">
          <label class="corCheckbox">
            <input type="checkbox"  id='semControleVagas' name='semControleVagas' value="1" {{$sc}}/>
    ...
    <div class='row'>
      ...
      <input id="dataInicioInscricao" type="date" class="validate" name='dataInicioInscricao' value={{$datainicio}} required>
    </div>
    <div class="input-field col s1">
      <span for="HoraInicio">Hora Inicio</span>
      <input id="HoraInicio" name='HoraInicio' type="text" class="timepicker" value={{$horainicio}} required>
    </div>
      <div class="input-field col s2">
        <span>Data Final das Inscrições</label>
        <input id="VencimentoPagamento" type="date" class="validate" name='VencimentoPagamento' value={{$datafim}} required >
      </div>
      <div class="input-field col s1">
        <span for="HoraFim">Hora Fim</span>
        <input id="HoraFim" name='HoraFim' type="text" class="timepicker" value={{$horafim}} required>
      </div>
    </div>
    <div class='row'>
      <div class='input-field col s12'>
