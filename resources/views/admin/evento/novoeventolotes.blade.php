  ...
  <form class='col s12' action='{{route('eventos.lotes')}}' method="POST" enctype="multipart/form-data">
      ...
      <div class='row'>
        <div class="input-field col s2">
          ...
          <input id="InicioLote" name='InicioLote' type="date" class="validate" required>
        </div>
        ...
        <div class="input-field col s2">
          ...
          <input id="FimLote" name='FimLote' type="date" class="validate" required>
        </div>
        <div class="input-field col s1">
...
<table>
    ...
    @foreach ($lista as $item)
      @php
          $dataInicio = date('d/m/Y H:i',strtotime($item->InicioLote));
          $dataFinal = date('d/m/Y H:i',strtotime($item->FimLote));
  ...
  <div class='row'>
    ...
    <div class='col s6' style="display: flex; justify-content: flex-end">
      <a class="btn waves-effect waves-light btnlarge" href='{{url('/eventos/formeventocupom/'. $idNovoEvento )}}'>Próxima etapa
