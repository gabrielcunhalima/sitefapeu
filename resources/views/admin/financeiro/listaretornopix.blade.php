...
<div class='row'>
    <form action='{{route('pix.carrega')}}' method="POST" enctype="multipart/form-data">
        ...
        <div class='input-field col offset-s4'>
            <span>Baixar PIX pagos até o dia:</label>
...
<table>
    ...
    @if (isset($dados)) 
        ...
        @endforeach
    @else
        <tr>NÃO HÁ REGISTROS DE PIX AINDA NÃO BAIXADOS ATÉ A DATA SELECIONADA</tr>
    @endif
    </tbody>
</table>
<BR><BR>
    @if ($quantidade > 0) 
        <form action='{{route('pix.retorno')}}' method="POST" enctype="multipart/form-data">
            <input type="hidden" name="_token" value="{{{ csrf_token() }}}" />
            <input id="dataCorte" type="hidden" class="center validate" name='dataCorte' value={{$dataCorte}}>
            <button type="submit" class="btn btn-primary btn-block">EXPORTAR PLANILHA</button><br>
        </form>
